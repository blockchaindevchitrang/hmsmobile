...
const BirthReportList = ({searchBreak, setSearchBreak, allData, onGetData}) => {
  ...
  const onAddPayRollData = async () => {
    try {
      if (caseId == '') {
        ...
        const urlData = `birth-report-store?case_id=${caseId}&doctor_id=${doctorId}&date=${moment(
          dateOfBirth,
  ...
  const onEditPayRollData = async () => {
    try {
      if (caseId == '') {
        ...
        setLoading(true);
        setErrorVisible(false);
        const urlData = `birth-report-update/${userId}?case_id=${caseId}&doctor_id=${doctorId}&date=${moment(
          dateOfBirth,
        ).format('YYYY-MM-DD')}&description=${description}`;
  ...
  const onDeletePayrollData = async id => {
    try {
      setLoading(true);
      const response = await onDeleteCommonApi(`birth-report-delete/${id}`);
      if (response.data.flag == 1) {
        onGetData();
        setLoading(false);
        setDeleteUser(false);
  ...
  const onGetSpecificDoctor = async id => {
    try {
      const response = await onGetSpecificCommonApi(`birth-report-edit/${id}`);
