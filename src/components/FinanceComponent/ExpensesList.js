...
const ExpensesList = ({searchBreak, setSearchBreak, allData, getData}) => {
  ...
  const onIncomeHeadGet = async () => {
    try {
      const response = await onGetCommonApi(`expense-head-get`);
  ...
  const onAddUsers = async () => {
    try {
      if (incomeHeadId == '') {
        ...
        formdata.append('expense_head', incomeHeadId);
        ...
        const response = await onAddExpenseApi(formdata);
  ...
  const onEditUsers = async () => {
    try {
      if (incomeHeadId == '') {
        ...
        const response = await onUpdateExpenseApi(formdata, userId);
  ...
  const onDeletePayrollData = async id => {
    try {
      setLoading(true);
      const response = await onDeleteCommonApi(`expense-delete/${id}`);
      if (response.status == 200) {
        getData();
        setLoading(false);
        setDeleteUser(false);
  ...
  const onGetSpecificDoctor = async id => {
    try {
      const response = await onGetSpecificCommonApi(`expense-edit/${id}`);
      if (response.status == 200) {
  ...
  const renderItem = ({item, index}) => {
    return (
      <View
        ...
        <Text style={[styles.dataHistoryText, {width: wp(33)}]}>
          {item.invoice_number}
        </Text>
                    ...
                    return (
                      <View style={styles.dropdown2BtnStyle2}>
                        {incomeHeadId != '' ? (
                          ...
                          <Text style={styles.dropdownItemTxtStyle}>
                            {selectedItem?.name || 'Select Expense Head'}
                          </Text>
                        )}
                      </View>
                    );
