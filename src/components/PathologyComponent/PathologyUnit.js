...
const PathologyUnit = ({
  ...
  const onAddPayRollData = async () => {
    try {
      if (eventTitle == '') {
        ...
        const urlData = `pathology-unit-store?name=${eventTitle}`;
  ...
  const onEditPayRollData = async () => {
    try {
      if (eventTitle == '') {
        ...
        setLoading(true);
        setErrorVisible(false);
        const urlData = `pathology-unit-update/${userId}?name=${eventTitle}`;
  ...
  const onDeletePayrollData = async id => {
    try {
      setLoading(true);
      const response = await onDeleteCommonApi(`pathology-unit-delete/${id}`);
      if (response.data.flag == 1) {
        onGetData();
        setLoading(false);
        setDeleteUser(false);
  ...
  const renderItem = ({item, index}) => {
    return (
      <View
        ...
        {unitAction.includes('edit') || unitAction.includes('delete') ? (
          <View style={styles.actionDataView}>
            {unitAction.includes('edit') && (
              <TouchableOpacity
                onPress={() => {
                  setUserId(item.id);
              ...
              <View style={styles.filterView}>
                {unitAction.includes('create') && (
                  <TouchableOpacity
                    ...
                    <Text style={styles.actionText}>New Pathology Unit</Text>
                  </TouchableOpacity>
                )}
              </View>
            )}
        ...
        <View style={styles.maneModalView}>
          ...
          <View style={styles.container}>
            <View style={styles.headerView}>
              <Text style={styles.headerText}>New Pathology Unit</Text>
