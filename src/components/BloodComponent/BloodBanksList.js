...
const BloodBanksList = ({
  ...
  const onAddBloodBank = async () => {
    try {
      if (eventTitle == '') {
        ...
        setErrorMessage('Please enter blood group.');
      } else if (departmentComment == '') {
        setErrorVisible(true);
        setErrorMessage('Please enter remained bags.');
      } else {
        setLoading(true);
  ...
  const onEditBloodBank = async () => {
    try {
      if (eventTitle == '') {
        ...
        setErrorVisible(false);
        setErrorMessage('');
        let dataUrl = `blood-bank-update/${userId}?blood_group=${eventTitle}&remained_bags=${departmentComment}`;
  ...
  const onDeletePayrollData = async id => {
    try {
      setLoading(true);
      const response = await onDeleteCommonApi(`blood-bank-delete/${id}`);
        ...
        <View style={styles.maneModalView}>
          ...
          <View style={styles.container}>
            <View style={styles.headerView}>
              <Text style={styles.headerText}>New Blood Group</Text>
            ...
            <Text
              style={[
                styles.titleText1,
                {marginTop: isPortrait ? hp(1.5) : wp(1)},
              ]}>
              {'Blood Group:'}
            </Text>
            <TextInput
            ...
            <Text
              style={[
                styles.titleText1,
                {marginTop: isPortrait ? hp(0) : wp(1.5)},
              ]}>
              {'Remained Bags:'}
            </Text>
            <TextInput
            ...
            <View style={styles.buttonView}>
              <TouchableOpacity
                onPress={() => {
                  userId != '' ? onEditBloodBank() : onAddBloodBank();
                }}
